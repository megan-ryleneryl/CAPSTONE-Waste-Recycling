...
try {
  if (process.env.FIREBASE_PRIVATE_KEY && process.env.FIREBASE_CLIENT_EMAIL) {
    ...
    // Development: try service account file
  ...
  console.warn('Firebase Admin SDK not initialized:', error.message);
  console.warn('Admin features (push notifications, auth management) will be unavailable.');
}
...
const FirebaseHelper = {
  ...
  // Get current timestamp
  getTimestamp() {
    return adminInitialized ? admin.firestore.Timestamp.now() : new Date();
  },

  // Get server timestamp (for Firestore)
  getServerTimestamp() {
    return adminInitialized ? admin.firestore.FieldValue.serverTimestamp() : new Date();
  },

  // Increment field helper
  increment(value = 1) {
    return adminInitialized ? admin.firestore.FieldValue.increment(value) : value;
  },

  // Array operations
  arrayUnion(...elements) {
    return adminInitialized ? admin.firestore.FieldValue.arrayUnion(...elements) : elements;
  },

  arrayRemove(...elements) {
    return adminInitialized ? admin.firestore.FieldValue.arrayRemove(...elements) : [];
  },
...
module.exports = {
  ...
  // Admin SDK — null if not initialized, so downstream checks like
  // `admin ? admin.messaging() : null` work correctly
  ...
  // Firebase utilities (if admin available)
  FieldValue: adminInitialized ? admin.firestore.FieldValue : null,
  Timestamp: adminInitialized ? admin.firestore.Timestamp : null
};
